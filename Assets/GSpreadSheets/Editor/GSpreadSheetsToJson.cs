...
public class GSpreadSheetsToJson : EditorWindow {
	...
	private void CreateJsonFile(string fileName, string outputDirectory, ValueRange valueRange)
	{
		...
		string find = nameof(json.datas);
        int index = jsonText.IndexOf(find);
        jsonText = index < 0 ? jsonText : jsonText.Substring(0, index) + fileName + jsonText.Substring(index + find.Length);
