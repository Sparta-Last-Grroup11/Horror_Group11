...
2) 점프스케어형:
- 특정 조건에서 발동하여 갑자기 튀어나오는 공포 연출, 무해한 몬스터
...
3) 순찰형:
- 일정 시간마다 랜덤 위치를 지정하여, 해당 위치로 이동하며 플레이어를 탐지
- 플레이어를 탐지하면, 플레이어에게 달려감
- 플레이어가 5초이상 감지 되지 않으면, 어그로가 풀림.
...
- 다국어 지원 (한글 로컬라이징): 텍스트 번역을 위한 로컬라이징 시스템 도입
<br>
...
### 💾 체크포인트 시스템
- 특정 위치에서 자동 저장되는 체크포인트 기능 구현  
- 게임오버 시 마지막 체크포인트에서 이어서 시작 가능  
- 컷씬 스킵과 함께 반복 플레이 피로도 감소
