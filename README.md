...
### 오브젝트 상호작용
1) 문제 상황<br>
- 전등 On/Off 시 몬스터가 정지/행동하는 기능을 구현하는 과정에서 동적 생성되는 몬스터에 접근할 방법이 없음<br>
2) 시도한 방법<br>
- 몬스터 직접 참조 -> 동적 생성이라 연결 불가능<br>
3) 해결 방법<br>
- ScriptableObject를 이용해 전원 상태를 저장<br>
- 상태 변화 시. Event를 통해 몬스터에게 상태 변경을 통지<br>
- 각 몬스터는 SO의 이벤트에 구독, 상태 변화 시 행동 상태를 변하도록 구현<br>
4) 결과<br>
